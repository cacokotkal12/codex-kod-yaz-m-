...
class ChatKomutMacroApp:
    ...
    def _send_input(self, scan_code, keyup=False):
        ...
        data = INPUT(type=INPUT_KEYBOARD, ii=INPUTUNION(ki=ki))
        self.SendInput(1, ctypes.byref(data), ctypes.sizeof(INPUT))

    def _key_tap(self, scan_code):
        self._send_input(scan_code, keyup=False)
        time.sleep(KEY_DELAY)
        self._send_input(scan_code, keyup=True)

    def _ctrl_v(self):
        self._send_input(SC_LCTRL, keyup=False)
        time.sleep(KEY_DELAY)
        self._key_tap(SC_V)
        time.sleep(KEY_DELAY)
        self._send_input(SC_LCTRL, keyup=True)
